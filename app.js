...
function findTopicForMessage(text) {
  if (!text || !Array.isArray(topicsIndex) || topicsIndex.length === 0) return null;

  const norm = (s) => (s ? normalize(String(s)) : "");
  const toArr = (x) => Array.isArray(x) ? x : (x == null ? [] : [x]);

  const q = norm(text);

  // 1) direct title match (exact / contains / starts-with)
  const direct = topicsIndex.find(topic =>
    topic && topic.title && (
      q === norm(topic.title) ||
      q.includes(norm(topic.title)) ||
      norm(topic.title).includes(q)
    )
  );
  ...
  for (const topic of topicsIndex) {
    ...
    const score = tokenOverlap(qTokens, new Set(tokenize(bag)));
    if (score > bestScore) { best = topic; bestScore = score; }
  }

  // require a minimal overlap to avoid bad matches
...
async function handleInfoLike(userText, scopedIntent) {
  ...
  // Helpers
  const symptomLike = /\b(symptoms?|signs?|warning\s+signs?)\b/i.test(t);
  const toArr = x => Array.isArray(x) ? x : (x == null ? [] : [x]);
  const badGenericIds = new Set(["antenatal_care_basics","medicines_in_pregnancy","pregnancy_nutrition","nutrition"]);

  // 0) Try “hint” path first (handles synonyms like “won’t latch”, “morning sickness”, etc.)
  const hintPath = resolveTopicFromHint(userText);
  if (hintPath) {
    const j = await fetchJsonSafe(hintPath);
    if (j) { await answerFromTopic(hintPath, userText); return; }
  }

  // 1) Try semantic topic from your index (title/keywords/aliases/tags)
  const topic = findTopicForMessage(userText);
  if (topic) {
    const j = await fetchJsonSafe(topic.path);
    if (j) { await answerFromTopic(topic.path, userText); return; }
  }

  // 2) Intent → topic mapping (only if the file exists AND is appropriate)
  const intentToTopic = {
  ...
  const mappedPath = intentToTopic[scopedIntent];
  if (mappedPath) {
    const j = await fetchJsonSafe(mappedPath);
    if (j) {
      // If user asked about symptoms/signs and the mapped file is a generic pack,
      // avoid serving it and fall through to web.
      const idGuess = String(mappedPath).split("/").pop().replace(".json","");
      if (!(symptomLike && badGenericIds.has(idGuess))) {
        await answerFromTopic(mappedPath, userText);
        return;
      }
    }
  }

  // 3) Nothing local fits → Web search (respect consent)
  if (searchOptIn) {
    await startWebSearch(userText);
    return;
  }
  awaitingSearchConsent = { text: userText };
  addMsg("I don’t have that in my local library. Want me to check trusted sources (WHO, SA DoH, UNICEF)?", "bot");
}
...
function composeFromTopicJSON(data, modes, userText) {
  const out = [];

  // helpers (safe for strings or arrays)
  const toArr  = (x) => Array.isArray(x) ? x : (x == null ? [] : [x]);
  const asText = (v) => String(v ?? "").trim();
  const pick   = (arr, n = 2) => toArr(arr).map(asText).filter(Boolean).slice(0, n);
  const stripRefs = (s) => String(s ?? "").replace(/:contentReference\[.*?\]/g, "").trim();

  // prefer short, practical general line when steps are asked
  const isPracticalLine = (s = "") =>
    /\b(latch|position|hold|tummy[-\s]?to[-\s]?tummy|wide\s+mouth|deep\s+latch|skin[-\s]?to[-\s]?skin|cue|on[-\s]?demand|burp|frequency|pain|sore)\b/i.test(s);
  const looksBenefit = (s = "") =>
    /\b(antibod|immun|6\s*months|exclusive|benefit|cancer|future\s+health)\b/i.test(s);

  // tiny helper: first sentence only
  ...
  if (modes.includes("definition") && data.definition) {
    out.push(firstSentence(stripRefs(data.definition)));
  }

  // 2) general — ONE short line; if steps requested, prefer practical over benefits
  if (modes.includes("general") && (data.general_info || data.symptoms)) {
    const allGeneral = pick(data.general_info ?? data.symptoms, 6).map(stripRefs);
    const wantSteps = modes.includes("steps");
    const chosen =
      (wantSteps && (allGeneral.find(isPracticalLine) || allGeneral.find(s => !looksBenefit(s)))) ||
      allGeneral.find(s => !looksBenefit(s)) ||
      allGeneral[0];

    if (chosen) out.push(firstSentence(chosen));
  }

  // 3) reassurance — ONE short line
  if (modes.includes("reassurance") && data.reassurance) {
    const rass = pick(data.reassurance, 1).map(stripRefs);
    if (rass.length) out.push(firstSentence(rass[0]));
  }

  // 4) steps — numbered, compact
  if (modes.includes("steps") && data.steps) {
    const steps = pick(data.steps, 3).map(stripRefs);
    if (steps.length) {
      const numbered = steps.map((s, i) => `${i + 1}. ${firstSentence(s)}`).join(" ");
      out.push("Here are a few simple steps: " + numbered);
    }
  }

  // 5) optional red flags if user hints at severity
  if (/\b(severe|danger|urgent|worse|emergency|fever|pus|ooze)\b/i.test(userText) && data.red_flags) {
    const flags = pick(data.red_flags, 2).map(stripRefs).map(firstSentence);
    if (flags.length) out.push("Get help urgently if: " + flags.join(", ") + ".");
  }

  // final: keep it tight (max ~4 sentences)
  const joined = out.join(" ");
  const parts = (joined.match(/[^.!?]+[.!?]/g) || [joined]).map(s => s.trim()).filter(Boolean).slice(0, 4);
  return parts.join(" ");
}
...
async function fetchJsonSafe(path) {
  try {
    ...
    if (!res.ok) {
      console.warn(`[fetchJsonSafe] ${path} -> ${res.status}`);
      return null;
    }
    return await res.json();
  } catch (e) {
    console.error("[fetchJsonSafe] error", e);
    return null;
  }
}
...
async function answerFromTopic(path, userText) {
  addMsg("Let me think about that…", "bot:thinking");

  const data = await fetchJsonSafe(path);
  if (!data) {
    if (searchOptIn) {
      addMsg("I don’t have that in my local notes — checking trusted sources for you…", "bot");
      return startWebSearch(userText);
    }
    awaitingSearchConsent = { text: userText };
    addMsg("I don’t have that in my local notes. Want me to check trusted sources (WHO, SA DoH, UNICEF)?", "bot");
    return;
  }
  // 🔒 Deterministic shape based on the question
  const modes = classifyAnswerMode(userText);
  console.log("[composer]", { modes, hasDef: !!data.definition, hasGen: !!data.general_info, hasReass: !!data.reassurance, hasSteps: !!data.steps });

  const msg = composeFromTopicJSON(data, modes, userText);
  ...
  const tooGeneric = /^most pregnancies|^a balanced diet|^medicines? in pregnancy/i.test(msg);
  if (!msg || tooGeneric) {
    console.log("[composer] generic/weak → web");
    if (searchOptIn) { await startWebSearch(userText); return; }
    awaitingSearchConsent = { text: userText };
    addMsg("I have only a general note on that. Want me to check trusted sources for details?", "bot");
    return;
  }

  // If JSON exists but fields are missing, fall back to a short, safe line
  addMsg("I have limited notes on that. Want me to check trusted sources for more detail?", "bot");
}




function classifyAnswerMode(userText) {
  const t = (userText || "").toLowerCase().trim();

  const askDef     = /^(what\s+is|define|meaning of)\b/.test(t);
  const askSigns   = /\b(symptoms?|signs?)\b/.test(t);
  // catches: how do I…, how should I…, how to…, steps, tips, manage, treat, help
  const askSteps   = /\b(what\s+should\s+i\s+do|how\s+(do|should)\s+i|how\s+to|steps?|tips?|manage|treat(ment)?|help)\b/.test(t);
  const askWorried = /\b(worried|scared|nervous|anxious|afraid|stressed|overwhelmed)\b/.test(t);

  if (askDef)     return ["definition", "general"];
  if (askSigns)   return ["general"];
  if (askSteps)   return ["general", "steps"];     // <- ensures “how do/should I breastfeed” shows steps
  if (askWorried) return ["general", "reassurance"];
  return ["general"];
}
...
form.addEventListener("submit", async (e) => {
  ...
  if (route.action === "basic_chat" && (specificInfo || clearlyMedical)) {
    // 1) explicit mapped file only if it exists
    const mappedPath = intentToTopic[forcedIntent];
    if (mappedPath) {
      const j = await fetchJsonSafe(mappedPath);
      if (j) {
        console.log("[forced-info] mapped", mappedPath);
        await answerFromTopic(mappedPath, text);
        return;
      } else {
        console.log("[forced-info] mapped path missing → web");
    ...
    // 2) semantic topic if not a generic fallback
    const topic = findTopicForMessage(text);
    const badIds = new Set(["antenatal_care_basics", "medicines_in_pregnancy", "nutrition"]);
    if (topic && !badIds.has(topic.id)) {
      const j = await fetchJsonSafe(topic.path);
      if (j) {
        console.log("[forced-info] semantic", topic);
        await answerFromTopic(topic.path, text);
        return;
      }
    }
  ...
  if (route.action === "basic_chat") {
    ...
    // 4) Info flows
  if (route.action === "info_local") {
    const hintedPath = resolveTopicFromHint(route.topic_hint);
    if (hintedPath) {
      const j = await fetchJsonSafe(hintedPath);
      if (j) {
        await answerFromTopic(hintedPath, text);
        return;
      }
      console.log("[info_local] hinted file missing → web");
      if (searchOptIn) { await startWebSearch(text); return; }
      awaitingSearchConsent = { text };
      addMsg("I don’t have that in my local notes. Want me to check trusted sources (WHO, SA DoH, UNICEF)?", "bot");
      return;
    ...
    // no hint? try semantic
    const topic = findTopicForMessage(text);
    if (topic) {
      const j = await fetchJsonSafe(topic.path);
      if (j) { await answerFromTopic(topic.path, text); return; }
    }
